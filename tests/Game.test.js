...
describe('Game Business Logic', () => {
    ...
    it('should update score correctly when clearing lines with combo multiplier', () => {
        game.start();
        const initialScore = game.score;
        
        // 1 line clear logic
        game.updateScoreAndLevel(1); 
        // 40 points per line at level 1, combo = 0 -> multiplier 1
        assert.strictEqual(game.score, initialScore + 40);
        
        // Let's add combo
        game.combo = 2; // combo = 2 -> multiplier 1 + (2 * 0.5) = 2.0
        
        game.updateScoreAndLevel(4);
        // 1200 points for 4 lines at level 1 * 2.0 = 2400
        assert.strictEqual(game.score, initialScore + 40 + 2400);
    });
});
