...
describe('ParticleManager Business Logic & Physics', () => {
    ...
    it('should initialize with an empty particles array and preset color palette', () => {
        ...
        assert.ok(particleManager.colors.includes('#ff1a40')); // карминовый неон
        assert.ok(particleManager.colors.includes('#ffb830')); // золото
    });
    ...
    it('should create specified number of particles on createFirework', () => {
        ...
        assert.strictEqual(particle.gravity, 0.25);
        assert.ok(particle.decay >= 0.02 && particle.decay <= 0.06);
    ...
    it('should trigger line clear explosion across multiple positions', async () => {
        ...
        // 5 explosions * 10 particles = 50 particles
        assert.strictEqual(particleManager.particles.length, 50);
        assert.strictEqual(container.children.length, 50);
    });
});
