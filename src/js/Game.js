...
import { ACTIONS, SHAPES, GAME_SPEED_START, MIN_SPEED, SPEED_INCREMENT, DRAX_QUOTES } from './constants.js';
...
export class Game {
    ...
    gameOver() {
        ...
        this.ui.toggleOverlay(true, 'Destroyed!', 'Re-Engage');
    }
            ...
            if (this.dropCounter >= this.dropInterval) {
                if (this.board.isValidMove(this.activePiece, this.activePiece.x, this.activePiece.y + 1)) {
                    ...
                    if (linesToClear.length > 0) {
                        ...
                        if (this.combo >= 2 || linesToClear.length >= 3) {
                            const quote = DRAX_QUOTES[Math.floor(Math.random() * DRAX_QUOTES.length)];
                            this.ui.showQuoteBanner(quote);
                        }
    ...
    updateScoreAndLevel(linesCleared) {
        ...
        this.score += Math.floor(lineScores[linesCleared] * this.level * comboMultiplier);
    ...
    hardDrop() {
        ...
        this.ui.triggerScreenShake('shake-lg');
