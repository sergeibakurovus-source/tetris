import { COLS, ROWS, DRAX_QUOTES } from './constants.js';
...
export class UIManager {
    constructor() {
        ...
        this.particles = new ParticleManager(this.boardContainer.parentElement);
    }
    ...
    updateCombo(combo) {
        this.comboIndicator.textContent = `${combo}x`;
        if (combo >= 2) {
            this.comboIndicator.classList.add('neonBreath');
        } else {
            this.comboIndicator.classList.remove('neonBreath');
        }
    }
    ...
    async animateClearLines(lines) {
        ...
        this.triggerScreenShake('shake-sm');
        ...
        return new Promise(resolve => {
            lines.forEach(r => {
                ...
                const containerRect = this.boardContainer.parentElement.getBoundingClientRect();
    ...
    triggerScreenShake(intensityClass = 'shake-sm') {
        this.gameWrapper.classList.remove('shake-sm', 'shake-lg');
        void this.gameWrapper.offsetWidth; // trigger reflow
        this.gameWrapper.classList.add(intensityClass);
        setTimeout(() => {
            this.gameWrapper.classList.remove(intensityClass);
        }, 500);
    }
    ...
    showQuoteBanner(quote) {
        ...
        this.quoteContainer.appendChild(banner);
